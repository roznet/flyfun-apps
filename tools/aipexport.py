...
from euro_aip.sources.austria_eaip_web import AustriaEAIPWebSource
...
class ModelBuilder:
    ...
    def _initialize_sources(self):
        ...
        if getattr(self.args, 'austria_web', False):
            self.sources['austria_eaip_web'] = AustriaEAIPWebSource(
                cache_dir=str(self.cache_dir),
                airac_date=self.args.airac_date
            )
...
def main():
    ...
    parser.add_argument('--austria-web', help='Enable Austria eAIP web source (PDF)', action='store_true')
    ...
    sources_enabled = any([
        ...
        args.uk_eaip, getattr(args, 'uk_web', False), getattr(args, 'norway_web', False), getattr(args, 'slovenia_web', False), getattr(args, 'austria_web', False),
