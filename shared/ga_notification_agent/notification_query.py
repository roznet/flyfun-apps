"""
Notification Query Tools - Query customs/immigration notification requirements.

Tools for querying the ga_notification_requirements database using natural language.
"""
...
def _get_airports_db_path() -> str:
    """Get path to the airports database."""
    web_server_path = Path(__file__).parent.parent.parent / "web" / "server" / "airports.db"
    if web_server_path.exists():
        return str(web_server_path)
    return str(web_server_path)


def _parse_weekday_rules(weekday_rules_json: Optional[str], day: str) -> Optional[str]:
    """
...
def get_notification_for_airport(
    ...
    **USE THIS TOOL when user asks about notification/customs requirements for a specific airport.**
    
    Examples:
    - "What's the notification for LFRG?" → use this tool
    - "When should I notify customs at LFPT for Saturday?" → use this tool with day_of_week="Saturday"
    ...
    """
    db_path = _get_notifications_db_path()
    ...
    try:
        ...
        airports_db = _get_airports_db_path()
        airport_coords = None
        airport_name = None
        if os.path.exists(airports_db):
            try:
        ...
        if day_of_week:
            day_rule = _parse_weekday_rules(row["weekday_rules"], day_of_week)
            if day_rule:
        ...
        if airport_coords:
            result["visualization"] = {
                ...
                "marker": {
                    ...
                    "zoom": 10,
                    "highlight": True,  # Blue highlight for mentioned airport
                    "style": "notification"
                }
            }
...
def find_airports_by_notification(
    ...
    **USE THIS TOOL when user asks for airports matching notification criteria.**
    
    Examples:
    - "Airports with less than 24h notice in France" → max_hours_notice=24, country="FR"
    - "Show me H24 airports in Germany" → notification_type="h24", country="DE"
    ...
    """
    notifications_db = _get_notifications_db_path()
    airports_db = _get_airports_db_path()
    ...
    try:
        ...
        if has_airports_db and country:
            ...
            # Always join airports_db to get coordinates even without country filter
            query = """
                SELECT n.icao, n.rule_type, n.notification_type, n.hours_notice,
                       n.weekday_rules, n.summary, n.confidence,
                       a.name as airport_name, a.municipality, a.iso_country,
        ...
        if max_hours_notice is not None:
            # Include H24 (no notice) and airports with hours_notice <= threshold
        ...
        for row in rows:
            airport = {
                "icao": row["icao"],
                "ident": row["icao"],  # Alias for map compatibility
                "notification_type": row["notification_type"],
                "hours_notice": row["hours_notice"],
        ...
        for apt in airports[:10]:  # Show first 10 in pretty output
            name = apt.get("name", "")
        ...
        return {
            ...
            "visualization": {
                "type": "markers",
                "data": airports,  # Show ALL matching airports on map
                "markers": airports,  # Will be filtered by formatting layer to only mentioned ICAOs
                "style": "customs"  # Use 'customs' style for blue highlight support in frontend
            }
        }
...
# Tool specifications for integration with aviation agent
NOTIFICATION_TOOL_SPECS = {
    "get_notification_for_airport": {
        "name": "get_notification_for_airport",
        "handler": get_notification_for_airport,
        "description": "Get customs/immigration notification requirements for a specific airport. Use when user asks about notification requirements, customs, or when to notify for a specific airport.",
        "parameters": {
            "type": "object",
            "properties": {
                "icao": {
    ...
    "find_airports_by_notification": {
        ...
        "description": "Find airports filtered by notification requirements. Use when user asks for airports with specific notice periods (e.g., '<24h notice') or notification types (H24, on_request).",
        "parameters": {
            "type": "object",
            "properties": {
                "max_hours_notice": {
...
if __name__ == "__main__":
    # Test the tools
    print("Testing get_notification_for_airport...")
    result = get_notification_for_airport("LFLY", day_of_week="Saturday")
    print(result["pretty"])
    print()
    
    print("Testing find_airports_by_notification...")
    result = find_airports_by_notification(max_hours_notice=24, country="FR", limit=5)
    print(result["pretty"])
    print()
    
    print("Testing get_notification_statistics...")
